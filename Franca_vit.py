...
class franca_vit:
    ...
    def load_model(self):
        model = torch.hub.load('valeoai/Franca', 'franca_vitb14', use_rasa_head=False)
    ...
    def operate_batched_image(self, image_batch):
        ...
        with torch.no_grad():
            feats = self.model.forward_features(image_batch.to(self.device), use_rasa_head=False)
    ...
    # def make_transform(self,resize_size: int = 518):
...
#     model = DinoV2_B_reg(model_name, model_path)
