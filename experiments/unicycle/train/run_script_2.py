...
class Autoencoder(nn.Module):
    def __init__(self, input_dim=768, latent_dim=50, bottleneck_dim=16):
        super().__init__()
        ...
        # -------- 2-layer nonlinear lift: 16 → 50 --------
        self.lift = nn.Sequential(
            nn.Linear(bottleneck_dim, 128),
            nn.ReLU(),
            nn.Linear(128, latent_dim),
            nn.LayerNorm(latent_dim),  # helps stabilize linear A,B
        )
    ...
    def encoder(self, x):
        h = self.trunk(x)
        z_core = self.to_bottleneck(h)  # 16D
        z = self.lift(z_core)           # 50D Koopman latent
        return z
...
class LatentDynamics(nn.Module):
    def __init__(self, latent_dim=20, control_dim=2):
        super().__init__()
        self.A = nn.Parameter(torch.eye(latent_dim) + 0.001 * torch.randn(latent_dim, latent_dim))
        self.B = nn.Parameter(0.01 * torch.randn(latent_dim, control_dim))

    def forward(self, z, u):
        # z: (B, latent_dim), u: (B, control_dim)
...
def train(
    ...
    h5_path = "/storage/scratch1/6/rjiang77/DINOv2_CLS/2_1_dataset/All/combined_dataset.h5"
    ...
    for epoch in range(num_epochs):
        ...
        for batch in loader:
            ...
            for t in range(N_T):
                # --- inside the rollout loop (same z_prev handling as above) ---
                z_prev = z_preds[-1]
...
if __name__ == "__main__":
    ...
    save_path = '/storage/scratch1/6/rjiang77/Autoencoder/3_15_2/'
